...
// --dry-run: every write below goes through these three, so a preview render
// cannot touch the working tree. The README is handed back via --out or stdout.
const DRY_RUN = args.includes("--dry-run");
const outIdx = args.indexOf("--out");
const dryRunOut = outIdx >= 0 ? args[outIdx + 1] : null;
const writeFileSync = DRY_RUN ? () => {} : fs.writeFileSync;
const mkdirSync = DRY_RUN ? () => {} : fs.mkdirSync;
const rmSync = DRY_RUN ? () => {} : fs.rmSync;
...
//
// renderQuickStart is the "how do I use it" answer that sits in the first
// ~2 KB of the README. LLM fetchers truncate at 10-50 KB and the full "Use the
...
function renderQuickStart(toolCount, toolMeta) {
  ...
  return `**Use it in one command.** Add it to Claude Code as an MCP server (Streamable HTTP, no key, no account):

\`\`\`sh
claude mcp add --transport http satohub ${SITE}/api/mcp
\`\`\`
...
function renderCommunity() {
  const lines = [
    CONFIG.x_url ? `- 𝕏 **[@SatoHub](${CONFIG.x_url})** — findings from the index` : null,
...
function renderSupport() {
  ...
  return `Donations fund the data infrastructure (daily pipelines, verification harness) and buy nothing: not a listing, not rank, not a score ([NEUTRALITY.md](NEUTRALITY.md)).\n\n${lines.join("\n")}`;
}

// ---------- runnable agent packages ----------
// Sealed, signed packages published on Sato Hub: a whole working agent rather
// than a component. Rendered from the public listings API so title, task,
// digest and evidence labels are the API's own words — nothing typed here.
// Kept small and early: this is what an LLM reader should reach before the
// README is truncated. A failed fetch drops the section; it never invents rows.

const LISTINGS_URL = `${SITE}/api/listings?limit=20`;
...
async function loadPackages() {
  try {
    ...
    return (body.listings || []).filter((l) => l && l.state === "published" && l.slug && l.version?.digest);
  } catch {
    return [];
  }
}

function packageEvidence(l) {
  const ev = l.evidence_freshness || {};
  const labels = (ev.labels || []).join(", ");
  if (!labels) return "no evidence recorded";
  return ev.count != null ? `${labels} · ${ev.count} platform row${ev.count === 1 ? "" : "s"}` : labels;
}

function renderPackages(listings) {
  if (!listings.length) return "";
  const rows = [...listings]
    .sort((a, b) => a.slug.localeCompare(b.slug))
    .map((l) => {
      const page = withUtm(l.sato_url || `${SITE}/agents/packages/${l.slug}`);
      const dl = `${SITE}/api/listings/${l.slug}/download`;
      const digest = l.version.digest;
      const short = digest.replace(/^sha256:/, "").slice(0, 12);
      return `| [${esc(l.title)}](${page}) | ${esc(l.task)} | <details><summary><code>sha256:${short}…</code></summary><code>${digest}</code></details> | \`${packageEvidence(l)}\` | [page](${page}) · [download](${dl}) |`;
    })
    .join("\n");
  return `<a id="runnable-agent-packages"></a>
## Runnable agent packages
...
Verify the download against the published key before you run it: [\`verify-release.mjs\`](${SITE}/verify-release.mjs) (\`node verify-release.mjs <package.zip> manifest.json\`).
`;
}
...
- [Neutrality](${raw}/NEUTRALITY.md): no project can pay to be listed, ranked or scored
...
`
  ...
  const tocExtras = [
    packages ? `- [Runnable agent packages](#runnable-agent-packages)` : null,
...
`;
  ...
  if (DRY_RUN) {
    if (dryRunOut) fs.writeFileSync(dryRunOut, readme);
    else process.stdout.write(readme);
  }
  ...
  // stdout carries the README in a dry run with no --out, so the summary goes to stderr then.
  (DRY_RUN && !dryRunOut ? console.error : console.log)(
    agg
      ? `${DRY_RUN ? "[dry-run, nothing written] " : ""}rendered ${resources.length} entries (${pagesChanged} pages written, ${pruned} pruned, ${agg.scored} scored, ${agg.independently_checked} independently checked, ${agents.length} passports${movers ? ", movers on" : ""}${support ? ", support on" : ""}${CONFIG.discord_url ? ", discord on" : ""})`
      : `${DRY_RUN ? "[dry-run, nothing written] " : ""}rendered ${resources.length} entries (${pagesChanged} pages written, ${pruned} pruned, ${scored} scored, ${verified} verified installs, ${agents.length} passports${movers ? ", movers on" : ""}${support ? ", support on" : ""}${CONFIG.discord_url ? ", discord on" : ""})`
  );
}
