...
The second door into the index (the first is the [add-project issue form](../../../issues/new?template=add-project.yml)). Both land in the Sato Hub review inbox. The two doors differ in one way: an issue is triaged automatically against public evidence and answered once on the issue; a proposal PR is reviewed by a person.
...
There is no bot reply on a proposal PR. When the listing goes live (or is declined with a one-line reason), a maintainer removes the file. (Issues opened with the add-project form are different: they get one automated reply with the evidence — see [CONTRIBUTING.md](../CONTRIBUTING.md).) Turnaround target: 48 hours for the first look.
